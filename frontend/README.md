...
| Frontend React (Web + Dashboard/Admin) | [`frontend`](frontend) | 5173 (dev) / 8080 (Docker) | — |
...
```bash
cd backend
docker compose up -d --build        # infraestructura + monolito + 3 microservicios + frontend
docker compose logs -f monolith     # la primera vez carga la data inicial (≈320 vuelos, ≈49.000 asientos)
# Frontend: http://localhost:8080
```

Frontend en modo desarrollo (con el backend ya levantado):
...
Detalle de instalación, prueba del tiempo real con dos pestañas y arquitectura del cliente en
